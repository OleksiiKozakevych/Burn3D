...
class visualize(QObject):
    ...
    def plot_data(self):
        ...
        preferences_json = parent_dir / "node_master" / "preferences.json"
        ...
        convert_to = {
                 ...
                 "Thrust Coefficient": "",
                 "Exit Pressure": "Pressure",
                 "Thrust": "Force"
                }
                 

        for key in ["Chamber Pressure", "Exit Pressure", "Thrust"]:
            series = QLineSeries()
            series.setName(key)
            print(key)
            print(convert_to[key])
            print(units[convert_to[key]])
            values = [v.to_base_units().to(units[convert_to[key]]).magnitude for v in results[key]]
